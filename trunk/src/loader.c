...
static void verifyClassFlags(u2 flags);
static void verifyFieldFlags(u2 flags, u2 classFlags);
static void loadStaticFieldAttributes(FILEPOINTER_HANDLE_FAR ClassFileH, INSTANCE_CLASS_FAR CurrentClass, FIELD_FAR thisField, POINTERLIST_HANDLE_FAR StringPoolH);
...
//#endif /* INCLUDEDEBUGCODE */
    ...
    * set the appropriate bit in the word */
    ...
    setWordAt(thisMethod.common_ptr_ + METHOD_ACCESSFLAGS, accessFlags);
    ...
    thisMethod = unhand(thisMethodH);
    if (!(/*thisMethod->accessFlags*/getWordAt(thisMethod.common_ptr_ + METHOD_ACCESSFLAGS) & (ACC_NATIVE | ACC_ABSTRACT))) {
...
static void loadMethodAttributes(FILEPOINTER_HANDLE_FAR ClassFileH,
    ...
    int attrIndex;
    BOOL needCode = !(getWordAt(thisMethod.common_ptr_ + METHOD_ACCESSFLAGS) & (ACC_NATIVE | ACC_ABSTRACT));
...
//        ASSERTING_NO_ALLOCATION
        {
            CONSTANTPOOL_FAR ConstantPool;
            getDWordAt(CurrentClass.common_ptr_ + INSTANCE_CLASS_CONSTPOOL);
            //if (USESTATIC) {
